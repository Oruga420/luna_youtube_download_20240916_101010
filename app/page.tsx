...
export default function HomePage() {
  ...
  const preferredItag = useMemo(() => {
    if (sortedFormats.length === 0) {
      return null;
    }

    const best = sortedFormats.find((format) => format.type === "video+audio");
    return best?.itag ?? sortedFormats[0].itag;
  }, [sortedFormats]);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setDetails(payload.details as VideoDetails);
      setFormats(payload.formats as VideoFormat[]);
      setSelectedItag((payload.formats as VideoFormat[])[0]?.itag ?? null);
      setStatus({
        type: "success",
